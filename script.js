...
function triggerUserListUpdate() {
  db.collection("users").orderBy("username").get().then((querySnapshot) => {
...
function updateUserList(users) {
  ...
  const filteredUsers = users.filter(user => {
    const matchSearch = user.username.toLowerCase().includes(query) ||
      (user.department || '').toLowerCase().includes(query) ||
      (user.section || '').toLowerCase().includes(query) ||
      (user.yearLevel || '').toLowerCase().includes(query);
    const matchDept = !departmentFilter || (user.department || '').toLowerCase() === departmentFilter;
    const matchYear = !yearFilter || (user.yearLevel || '').toLowerCase() === yearFilter;
  ...
  filteredUsers.forEach((user) => {
    ...
    const isActive = user.lastSeen?.seconds && (Date.now() - user.lastSeen.seconds * 1000) < 60000;
    const status = isActive
      ? '<span class="text-green-500 font-medium">Active</span>'
      : '<span class="text-gray-500 font-medium">Inactive</span>';

      tr.innerHTML = `
      <td class="px-4 py-2"><span contenteditable="false" id="username-${user.id}">${user.username}</span></td>
      <td class="px-4 py-2"><span contenteditable="false" id="password-${user.id}">${user.password}</span></td>
      ...
      <td class="px-4 py-2 whitespace-nowrap"><span contenteditable="false" id="section-${user.id}" class="inline-block w-24 truncate">${user.section || ''}</span></td>
      <td class="px-4 py-2 whitespace-nowrap"><span contenteditable="false" id="year-${user.id}" class="inline-block w-24 truncate">${user.yearLevel || ''}</span></td>
      <td class="px-4 py-2">${status}</td>
      <td class="px-4 py-2">
        <div class="flex space-x-2">
          <button onclick="enableEditing('${user.id}', '${user.role}')" class="text-blue-600 hover:underline">Edit</button>
...
function enableEditing(userId, role) {
  ...
  if (role === 'program_head' || role === 'teacher') deptSpan.contentEditable = true;
  if (role === 'student') {
  ...
  saveBtn.onclick = () => {
    ...
    if (role === 'program_head' || role === 'teacher') updateData.department = deptSpan.textContent.trim();
    if (role === 'student') {
  ...
  const actionCell = usernameSpan.parentElement.parentElement.querySelector('td:last-child');
  actionCell.appendChild(saveBtn);
}

window.addEventListener('DOMContentLoaded', () => {
  if (typeof firebase === 'undefined') {
    alert('Firebase failed to load. Please check firebase.js and script order.');
    return;
  }
